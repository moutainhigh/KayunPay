...
public class Report extends BaseController {
	...
	public void createRegText() throws Exception {
		...
		// 查询当日开户所需报备数据
		List<Object[]> obj = Db.query(
				"SELECT t2.ssn,t1.userName,t1.loginId,t2.trueName,t3.userCardId,t1.userMobile,t2.modifyDateTime FROM t_user t1,t_banks_v2 t2,t_user_info t3 WHERE t1.userCode = t2.userCode AND t1.userCode = t3.userCode AND t2.modifyDateTime >= ? AND t2.modifyDateTime <= ? AND loginId <> '' AND ssn <> '' ORDER BY modifyDateTime ASC",
						startTime, endTime);
		...
		if (obj == null || obj.size() == 0) { // 如果当日无开户记录，也要报备一个空文件
			...
			for (int i = 0; i < obj.size(); i++) {
				...
				String mchnt_txn_ssn = ((String) tmp[0]); // 平台注册流水
				...
				String mobileNum = CommonUtil.decryptUserMobile((String) tmp[5]); // 手机号
	...
	public void createProjectText() {
		...
		List<LoanInfo> list = LoanInfo.loanInfoDao.find(
				"SELECT * FROM t_loan_info WHERE releaseDate BETWEEN ? AND ? AND createDate >= 20170816",
						startDate, endDate);
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					loanUserLoginId = CommonUtil.decryptUserMobile(user
							.getStr("loginId"));
					if (loanUserLoginId == null && i < (list.size() - 1)) {
						continue;
					}
				} catch (Exception e) {
				...
				if (i == list.size() - 1 || (i % 1999 == 0 && i != 0)) {
					try {
						transNo = String.valueOf(No);
						NoString = buqi[transNo.length()] + transNo;
						fileName = "P2P_PWXM_" + yesterday + "_" + NoString + ".txt";
						File f = new File(CommonUtil.reportPath + fileName);
						if (!f.exists()) {
							f.createNewFile();
						}
	...
	public void createTradeText() throws Exception {
		...
		// 投标交易(冻结)
		List<FuiouTrace> listDJ = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType = 'C' AND traceDate >= ? AND traceDate <= ? ORDER BY traceDate ASC",
						startTime, endTime);
		if (!(listDJ == null || listDJ.size() == 0)) {
			for (int i = 0; i < listDJ.size(); i++) {
				...
				UserInfo userInfo = UserInfo.userInfoDao.findById(tmp.getStr("InUserCode"));
		...
		// 投标交易(解冻)
		List<FuiouTrace> listJD = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType = 'D' AND traceDate >= ? AND traceDate <= ? ORDER BY traceDate ASC",
						startTime, endTime);
		...
		// 平台资金迁移到存管
		List<FuiouTrace> listQY = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType = 'E' AND traceDate >= ? AND traceDate <= ? ORDER BY traceDate ASC",
						startTime, endTime);
		...
		// 满标放款
		List<FuiouTrace> manBiao = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType = 'R' AND traceDate >= ? AND traceDate <= ? AND loanCode <> '' ORDER BY traceDate ASC",
						startTime, endTime);
		if (!(manBiao == null || manBiao.size() == 0)) {
			for (int i = 0; i < manBiao.size(); i++) {
				...
				String tradeNo = loanInfo.getStr("loanNo");// 借款标编号
				String contractNo = loanInfo.getStr("contractNo"); // 合同编号

				String outLogin_id = tmp.getStr("OutLoginId"); // 出账人存管系统用户名
				UserInfo userInfo = UserInfo.userInfoDao.findById(tmp.getStr("OutUserCode"));
				String outTrueName = userInfo.getStr("userCardName").trim(); // 出账人存管系统真实姓名
				long amt = tmp.getLong("amount"); // 交易金额,单位(分)
				String fees = "0"; // 手续费
				String inLogin_id = tmp.getStr("InLoginId"); // 入账人存管系统用户名
				UserInfo userInfo2 = UserInfo.userInfoDao.findById(tmp.getStr("InUserCode"));
		...
		// 债权转让
		List<LoanTransfer> transfers = LoanTransfer.loanTransferDao.find(
				"select * from t_loan_transfer where transState = 'B' and CONVERT(CONCAT(gotDate,gotTime),SIGNED) >= ? AND CONVERT(CONCAT(gotDate,gotTime),SIGNED) <= ? ORDER BY uid ASC",
						startTime, endTime);
		if (!(transfers == null || transfers.size() == 0)) {
			for (int i = 0; i < transfers.size(); i++) {
				...
				FuiouTrace fuiouTrace = FuiouTrace.fuiouTraceDao.findFirst(
						"SELECT * FROM t_fuiou_trace WHERE loanCode = ? AND InUserCode = ? AND OutUserCode = ?",
								loanCodeString, inUserCodeString, outUserCodeString);
				if (fuiouTrace != null) {
					mchnt_txn_ssn = fuiouTrace.getStr("traceCode"); // 项目流水号
				}
				...
				UserInfo userInfo = UserInfo.userInfoDao.findById(user.getStr("userCode"));
				String outLogin_id = CommonUtil.decryptUserMobile(user.getStr("loginId")); // 出账人存管系统用户名
				...
				UserInfo userInfo2 = UserInfo.userInfoDao.findById(tmp.getStr("payUserCode"));
				String inLogin_id = CommonUtil.decryptUserMobile(user2.getStr("loginId")); // 入账人存管系统用户名
		...
		// 平台手续费
		List<FuiouTrace> feeList = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType IN('V','I') AND traceDate BETWEEN ? AND ?",
						startTime, endTime);
		...
		// 平台代偿
		List<FuiouTrace> repay = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType IN('HKB','HKL') AND traceDate >= ? AND traceDate <= ? AND OutUserCode <> ''",
						startTime, endTime);
		...
		// 平台营销
		List<FuiouTrace> list = FuiouTrace.fuiouTraceDao.find(
				"SELECT * FROM t_fuiou_trace WHERE traceType  in ('Q','HFK') AND traceDate BETWEEN ? AND ?",
						startTime, endTime);
