...
public class FuiouPayController extends BaseController {
	...
	private static final Logger fuiouPayLogger = Logger.getLogger("fuiouPayLogger");
	
	static {
		LoggerUtil.initLogger("fuiouPayLogger", fuiouPayLogger);
	}
	...
	public Message fastByFuiou() {
		...
		if (true) {
			return error("01", "恒丰银行快捷充值接口已关闭", null);
		}
	...
	public Message wyByFuiou() {
		...
		if (true) {
			return error("01", "恒丰银行网银充值通道已关闭", null);
		}
	...
	public void rechargeAsync() {
		try {
			...
			if (trace == null) {
				fuiouPayLogger.log(Level.INFO, FuiouRspParseService.notifyRspXml("0001", mchnt_cd, mchnt_txn_ssn));
			...
			if ("ON".equals(FuiouPayController.SWITCH)) {
				...
				if (mchnt_cd == null || !mchnt_cd.equals(CommonUtil.MCHNT_CD)) {
					// 记录日志
					BIZ_LOG_ERROR(userCode, BIZ_TYPE.RECHARGE, "充值失败_商户号不符", null);
					fuiouPayLogger.log(Level.INFO, String.format("用户[%s]充值失败，商户号不符", userCode));
				...
				if (amt == null || payAmount != trace.getLong("traceAmount")) {
					// 记录日志
					BIZ_LOG_ERROR(userCode, BIZ_TYPE.RECHARGE, "充值失败_充值金额不符", null);
					fuiouPayLogger.log(Level.INFO, String.format("用户[%s]充值失败，充值金额不符", userCode));
				...
				try {
					...
					// 记录日志
					BIZ_LOG_ERROR(userCode, BIZ_TYPE.RECHARGE, "充值短信发送失败", null);
					fuiouPayLogger.log(Level.INFO, "充值短信发送失败");
				...
				if (trace.update()) {
					...
					fuiouPayLogger.log(Level.INFO, String.format("[%s]充值异步修改资金[%s]", userCode, payAmount));
					fuiouPayLogger.log(Level.INFO, FuiouRspParseService.notifyRspXml("0000", mchnt_cd, mchnt_txn_ssn));
					renderHtml(FuiouRspParseService.notifyRspXml("0000", mchnt_cd, mchnt_txn_ssn));
					FuiouPayController.SWITCH = "ON";
					return ;
				} else {
					// 记录日志
					BIZ_LOG_ERROR(userCode, BIZ_TYPE.RECHARGE, "充值流水更新失败", null);
					fuiouPayLogger.log(Level.INFO, String.format("[%s]充值流水更新失败", userCode));
	...
	public void fast_showResult4fuiouForP() {
		try {			
			...
			if (trace == null) {
				redirect(CommonUtil.APP_URL+"/ispaysucc.html?type=payerro",true);
			}
			...
			if ("0000".equals(resp_code)) {
				...
				if (trace.update()) {
					...
					fuiouTraceService.fuiouTraceContent(mchnt_txn_ssn,
							user, payAmount, FuiouTraceType.A);
					redirect(CommonUtil.APP_URL+"/ispaysucc.html?type=paysucc", true);
				} else {
					// 记录日志
					BIZ_LOG_ERROR(userCode, BIZ_TYPE.RECHARGE, "充值流水更新失败", null);
					redirect(CommonUtil.APP_URL+"/ispaysucc.html?type=payerro",true);
