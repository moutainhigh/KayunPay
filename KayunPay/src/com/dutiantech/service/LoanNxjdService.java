...
public class LoanNxjdService extends BaseService {
	...
	/**
	 * 根据ztyLoanCode查询最后一个关联标 WJW
	 * @param ztyLoanCode
	 * @return
	 */
	public LoanNxjd findLastByZtyLoanCode(String ztyLoanCode){
		return LoanNxjd.loanNxjdDao.findFirst("SELECT * FROM `t_loan_nxjd` WHERE ztyLoanCode = ? order by CONCAT(addDate,addTime) desc", ztyLoanCode);
	}
	 ...
	 * 易分期总数量
	 * */
	public long sumLoanNxjdNum(){
		return Db.queryLong("select count(*) from t_loan_nxjd");
	}
}
