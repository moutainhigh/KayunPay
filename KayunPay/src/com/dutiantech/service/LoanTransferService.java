...
//		Long lixi = CommonUtil.f_004(leftAmount, rate , loanTrace.getInt("loanRecyCount"), loanTrace.getStr("refundType"))[1];
	...
	* */
	...
	/**
	 * 更改jxTraceCode,orderId WJW
	 * @param jxTraceCode
	 * @param orderId
	 * @param transCode
	 * @return
	 */
	public boolean updateJxTraceCodeAndOrderId(String jxTraceCode,String orderId,String transCode){
		return Db.update("update t_loan_transfer set jxTraceCode=?,orderId=? where transCode=?",jxTraceCode,orderId,transCode) > 0;
	}
	 ...
	 * 分页查询逾期债权转让列表
	 * */
	public Page<LoanTransfer> queryOverdueLoanTransfer(int pageNumber,int pageSize,String transState,String transStatus,String orderType,String orderParam,String payUserCode,String refundType){
		String sqlSelect = "select * ";
		String sqlFrom = "from t_loan_transfer ";
		String sqlOrder = "";
//		String sqlWhere = "where loanCode in (select loanCode from t_loan_overdue where disposeStatus = 'n' and disposeDateTime = '' ) ";
		...
		if("1".equals(transStatus)){
			sqlWhere += "and jxTraceCode is null ";
		}else if("2".equals(transStatus)){
			sqlWhere += "and jxTraceCode is not null ";
		}
		if(refundType.equals("none")){
			sqlWhere += " and traceCode in (select traceCode from t_loan_trace where (loanState='N' or overdueAmount>0) and authCode is null) ";
		}else{
			sqlWhere += " and traceCode not in (select traceCode from t_loan_trace where (loanState='N' or overdueAmount>0) and authCode is null) ";
			if(!StringUtil.isBlank(refundType)){
				makeExp(buff, ps, "refundType", "=", refundType, "and");
			}
		}
		...
		if((StringUtil.isBlank(orderParam)&&!"B".equals(transState))||"allRate".equals(orderParam)){
			...
			if("DESC".equals(orderType)){
				sqlOrder = " order by transDate desc,transTime desc,uid desc ";
			}else{
				sqlOrder = " order by transDate asc,transTime asc,uid asc ";
			}
		}else if("transState".equals(orderParam)){
			sqlOrder = " order by transState asc,transDate desc,transTime desc ";
		}else{
...
//		String sqlWhere = "where loanCode in (select loanCode from t_loan_overdue where disposeStatus = 'n' and disposeDateTime = '' ) ";
		...
		if((StringUtil.isBlank(orderParam)&&!"B".equals(transState))||"allRate".equals(orderParam)){
			...
			if("B".equals(transState)){
				if("DESC".equals(orderType)){
					sqlOrder = " order by gotDate desc,gotTime desc,uid desc ";
				}else{
					sqlOrder = " order by gotDate asc,gotTime asc,uid asc";
				}
			}else{
				if("DESC".equals(orderType)){
					sqlOrder = " order by transDate desc,transTime desc,uid desc ";
	...
	/**
	 * 按日期 状态查询债权转让笔数   时间范围内 20181031 hw
	 */
	public long queryTransferByDate2(String start,String end ,String transState){
		if(StringUtil.isBlank(transState)){
			return Db.queryLong("select count(transCode) from t_loan_transfer where (transDate between ? and ?) or (gotDate between ? and ?)",start,end ,start,end);
		}else{
			return Db.queryLong("select count(transCode) from t_loan_transfer where transState = ? and  ((transDate between ? and ?) or (gotDate between ? and ?))",transState,start,end,start,end);
		}
	}
	/**
	 * 查询昨日承接5折债转的编号  20181031 hw
	 */
	public long[] numTransferFive(String yesterdayGotDate){
		String sql = "select count(*) from t_loan_transfer where transState='B' and refundType = 'H' and gotDate <= ? ";//查询截止昨天的编号
		Long yesterdayNum = Db.queryLong(sql,yesterdayGotDate);//查询截止昨天的编号
		
		String sql2 = "select count(*) from t_loan_transfer where transState='B' and refundType = 'H' and gotDate = ?";
		Long yesNum = Db.queryLong(sql2,yesterdayGotDate);//昨天承接得数量
		//昨天最大编号H
		long maxNum = yesterdayNum;
		//昨天最小的编号
		long minNum = yesterdayNum - yesNum + 1;
	 ...
	 * @return
	 */
	public LoanTransfer findByOrderId(String orderId){
		String sql = "select * from t_loan_transfer where orderId=?";
		return LoanTransfer.loanTransferDao.findFirst(sql,orderId);
	}
	
	/**
	 * 根据债转流水状态,承接日期,是否含有jxTraceCode查询不同refundType对应资金总额 WJW
	 * @param transState	承接状态
	 * @param gotDate		承接日期
	 * @param isJxTraceCode	null:不作为查询条件,true:jxTraceCode不为null,false:为null
	 * @param isAuthCode 	null:不作为条件查询,true:authCode不为null,false:为null
	 * @return
	 */
	public List<LoanTransfer> queryLoanTransfer(String transState,String gotDate,Boolean isJxTraceCode,Boolean isAuthCode){
		String sqlFrom = "select refundType,COALESCE(sum(transAmount),0) sumTransAmount from t_loan_transfer";
		String sqlWhere = " where transState=?";
		String sqlGroupby = " group by refundType";
		if(!StringUtil.isBlank(gotDate)){
			sqlWhere += " and gotDate='" + gotDate+"'";
		}
		if(isJxTraceCode != null){
			if(isJxTraceCode){
				sqlWhere += " and jxTraceCode is not null";
			}else {
				sqlWhere += " and jxTraceCode is null";
		...
		if(isAuthCode != null){
			if(isAuthCode){
				sqlWhere += " and authCode is not null";
			}else {
				sqlWhere += " and authCode is null";
			}
		}
		return LoanTransfer.loanTransferDao.find(sqlFrom + sqlWhere + sqlGroupby,transState);
	}
