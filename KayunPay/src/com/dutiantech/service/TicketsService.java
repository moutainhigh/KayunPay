...
//		UserInfo user = UserInfo.userInfoDao.findById( userCode );
	...
	public List<Tickets> QueryTicketsByCondition(Map<String, String>... condition) {
		String sqlSelect = "SELECT " + basic_selectFields;
		String sqlFrom = " FROM t_tickets";
		String sqlOrder = " ORDER by makeDateTime desc";
		
		List<Object> paras = new ArrayList<Object>();
		StringBuffer buff = new StringBuffer("");
		for (Map<String, String> map : condition) {
			if (!StringUtil.isBlank(map.get("userCode"))) {
				makeExp(buff, paras, "userCode", "=", map.get("userCode"), "and");
			}
			if (!StringUtil.isBlank(map.get("tname"))) {
				makeExp(buff, paras, "tname", "=", map.get("tname"), "and");
			}
			if (!StringUtil.isBlank(map.get("tstate"))) {
				makeExp(buff, paras, "tstate", "=", map.get("tstate"), "and");
			}
			if (!StringUtil.isBlank(map.get("rate"))) {
				makeExp(buff, paras, "rate", "=", map.get("rate"), "and");
			}
		}
		return Tickets.ticketsDao.find(sqlSelect + " " + sqlFrom + (makeSql4Where(buff)).toString() + sqlOrder, paras.toArray());
	}
	
	public long queryTicketsBytnameAndDate(String userCode,String tname,String startDateTime,String endDateTime){
		if(startDateTime.length()==8){
			startDateTime = startDateTime + "000000"; 
		}
		if(endDateTime.length()<14){
			endDateTime = DateUtil.getNowDateTime(); 
		}
		return Db.queryLong("select count(*) from t_tickets where userCode=? and tname = ? and makeDateTime between ? and ?",userCode,tname,startDateTime,endDateTime);
	}
	 ...
	 * @return
	 */
	public Long countByTtypeAndTstate(String userCode,String ttype,String tstate){
		return Db.queryLong("select count(1) from t_tickets where userCode = ? and ttype = ? and tstate = ? ",userCode,ttype,tstate);
	}
	...
	public List<Tickets> findYFByCode(String tCode){
		String sql="select * from t_tickets where tname like '%YF%' and tCode=?";
		return Tickets.ticketsDao.find(sql,tCode);
	}
	
	/**
	 * 根据userCode查询用户通过邀新所获得的投资券的数量
	 * @param userCode
	 * @return
	 */
	public Integer findTicketsByUserCode(String userCode){
		Tickets tickets = Tickets.ticketsDao.findFirst("SELECT COUNT(1) tNum FROM t_tickets where userCode = ? and tname REGEXP '好友实名|邀新|邀请好友'", userCode);
		return tickets.getLong("tNum").intValue();
	}
}
